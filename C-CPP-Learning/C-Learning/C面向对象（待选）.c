// 虽然我们的教材有这么一个结论：
// C语言是面向过程的语言，C++是面向对象的编程语言，但面向对象的概念是在C语言阶段就有了，而且应用到了很多地方，
// 比如某些操作系统内核、通信协议等。

// 面向对象编程，也就是大家说的OOP（Object Oriented Programming）并不是一种特定的语言或者工具，
// 它只是一种设计方法、设计思想，它表现出来的三个最基本的特性就是封装、继承与多态。
...
// 所以这个问题其实很好理解，只要有一定C语言编程经验的读者都应该能明白：
// 面向过程的C语言和面向对象的C++语言相比，代码运行效率、代码量都有很大差异。
// 在性能不是很好、资源不是很多的MCU中使用C语言面向对象编程就显得尤为重要。

// 2、所具备的条件
// 要想使用C语言实现面向对象，首先需要具备一些基础知识。
// 比如：（C语言中的）结构体、函数、指针，以及函数指针等，
// （C++中的）基类、派生、多态、继承等。

// 首先，不仅仅是了解这些基础知识，而是有一定的编程经验，
// 因为上面说了“面向对象是一种设计方法、设计思想”，
// 如果只是停留在字面意思的理解，没有这种设计思想肯定不行。
...
// 整个例子，非常简单，非常好理解。
// 以后写代码时候，要多去想想标准库的文件IO操作，这样也有意识的去培养面向对象编程的思维。

// 4、继承
// 继承就是基于现有的一个类去定义一个新类，这样有助于重用代码，更好的组织代码。
// 在 C 语言里面，去实现单继承也非常简单，只要把基类放到继承类的第一个数据成员的位置就行了。

// 例如，我们现在要创建一个 Rectangle 类，
// 我们只要继承 Shape 类已经存在的属性和操作，再添加不同于 Shape 的属性和操作到 Rectangle 中。
...
// 因为有这样的内存布局，所以你可以很安全的传一个指向 Rectangle 对象的指针到一个期望传入 Shape 对象的指针的函数中，
// 就是一个函数的参数是 “Shape *”，你可以传入 “Rectangle *”，并且这是非常安全的。
...
// 虚指针（Virtual Pointer）是一个指向虚表的指针。
// 这个虚指针必须存在于每个对象实例中，会被所有子类继承。
