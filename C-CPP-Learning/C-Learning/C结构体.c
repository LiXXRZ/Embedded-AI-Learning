// C99标准下的Struct的初始化方法
// 我们在阅读GNU/Linux内核代码时，我们会经常遇到这样一种特殊的结构初始化方式，这种方式称为指定初始化。
// 这种初始化方法源自C99标准。
...
// Linux内核喜欢用.fieldname=value的方式进行初始化
// 使用指定初始化，一个明显的优点是成员初始化顺序和个数可变，并且扩展性好，比如增加字段时，避免了传统顺序初始化带来的大量修改。
...
// 这行代码的作用是：
// 创建struct A类型的对象，变量名为a；
// 调用构造函数A(int a, int b)，并传入参数1和2；
// 构造函数执行后，对象a的成员变量a被赋值为1，成员变量b被赋值为2。
...
// 数组里的对象，不论是类对象（struct或者class），还是基础数据类型，如果没有给初始化列表，一律执行值初始化
// 即基础数据类型都初始化为0，类对象执行默认构造函数（即不接受任何参数的构造函数）。

// 结构体的赋值

// 结构体变量的赋值和初始化是不一样的，初始化是在变量定义的时候完成的，是属于变量定义的一部分
// 赋值是在变量定义完成之后想改变变量值的时候所采取的操作。
...
// 结构体知识 - 菜鸟教程
// 1、概述
// C 语言允许用户自己指定这样一种数据结构，它由不同类型的数据组合成一个整体，以便引用
// 这些组合在一个整体中的数据是互相联系的，这样的数据结构称为结构体，它相当于其它高级语言中记录。
...
// 为了能在程序中使用结构类型的数据，应当定义结构体类型的变量，并在其中存放具体的数据，可以采取以下3种方法定义结构体类型变量。
...
// 应当注意，将一个变量定义为标准类型（基本数据类型）与定义为结构体类型不同之处在于后者不仅要求指定变量为结构体类型，而且要求指定为某一特定的结构体类型（例如 struct student 类型）
// 因为可以定义出许多种具体的结构体类型。而在定义变量为整形时，只需指定为 int 型即可。
...
// 先声明一个 struct date 类型，它代表 日期 包括3个成员 month, day, year。
// 然后在声明 struct student 类型时，将成员 birthday 指定为 struct date 类型。
...
// 例如 student1.num 表示 student1 变量中的 num 成员
// 即 student1 的 num 项，可以对变量的成员赋值。例如： student1.num = 10010;

// . 是成员（分量）运算符，它在所有的运算符中优先级最高，因此可以把 student1.num 作为一个整体来看待。
// 上面的赋值语句作用是将整数 10010赋给 student1 变量中的成员 num。
...
// 在主函数中声明了 struct student 类型，然后定义了一个 struct student 类型的变量
// stu_1 同时又定义一个指针变量 p ，它指向一个 struct student 类型的数据
// 在函数的执行部分将结构体变量 stu_1 的起始地址赋给指针变量 p ，也就是使 p 指向 stu_1 然后对 stu_1 的各成员赋值
// 第二个 printf 函数也是用来输出 stu_1 各成员的值，但使用的是 (*p).num 这样的形式
// (*p) 表示 p 指向的结构体变量，(*p).num 是 p 指向的结构体变量中的成员 num
// 注意 p 两侧的括弧不可省略，因为成员运算符 '.' 优先于 '' 运算符，*p.num 就等价于 *(p.num)
...
// 在C语言中，为了使用方便和使之直观，可以把 (*p).num 改用 p->num 来代替
// 它表示 *p 所指向的结构体变量中的 num 成员，同样，(*p).name 等价于 p->name。
...
// （2）程序已定义了指针 p 为指向 struct student 类型数据的变量
// 它只能指向一个 struct student 型的数据（p 的值是 stu 数组的一个元素的起始地址），而不能指向 stu 数组元素中的某一成员，（即 p 的地址不能是成员地址）
// 例如，下面是不对的：
...
// （1）用结构体变量的成员作参数，例如：用 stu[1].num 或 stu[2].name 作函数实参，将实参值传给形参。
// 用法和用普通变量作实参是一样的，属于 值传递 方式。应当注意实参与形参的类型保持一致。
// （2）用结构体变量作参数。老版本的C系统不允许用结构体变量作实参，ANSI C取消了这一限制。
// 但是用结构体变量作实参时，采取的是 值传递 的方式，将结构体变量所占的内存单元全部顺序传递给形参。形参也必须是同类型的结构体变量。
// 在函数调用期间形参也要占用内存单元。这种传递方式在空间和时间上开销较大，如果结构体的规模很大时，开销是很可观的
// 此外由于采用值传递方式，如果在执行被调用函数期间改变了形参（也是结构体变量）的值，该值不能返回主调函数，这往往造成使用上的不便。因此一般较少用这种方法。
...
// 可以看出，头指针 head 指向第一个元素，第一个元素又指向第二个元素...直到最后一个元素
// 该元素不再指向其他元素，它称为 表尾，它的地址部分放一个 NULL（表示 空地址）链表到此结束。

// 可以看到链表中各元素在内存中可以不是连续存放的，要找某一元素，必须先找到上一个元素，根据它提供的下一元素地址才能找到下一个元素
// 如果不提供 头指针 head 则整个链表无法访问

// 可以看到，这种链表的数据结构，必须利用指针变量才能实现
// 即一个结点中应包含一个指针变量，用它存放下一结点的地址

// 前面介绍了结构体变量，用它作链表中的结点是最合适的
// 一个结构体变量包含若干成员，这些成员可以是数值类型，字符类型，数组类型，也可以是指针类型
// 我们用这个指针类型成员来存放下一个结点的地址。例如可以设计这样一个结构体类型：
...
// 现在 next 是 struct student 类型中的一个成员，它又指向 struct student 类型的数据
// 用这种方法就可以建立链表
...
// free 函数无返回值
// 请注意：以前的C版本提供的 malloc 和 calloc 函数得到的是指向字符型数据的指针
