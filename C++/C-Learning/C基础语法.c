...
// image-20220408161129665

// 既然指针在操作系统中都是用8个字节表示一个地址，那么为什么定义的时候需要声明什么类型的指针变量呢？

// 声明类型以后，在p+1的时候，编译器才知道应该跨越多少字节（如指向a[1]，a[1][1] 的指针在加1后偏移的地址不同）才能指向下一个变量的地址。
...
// image-20220407210221512

// 没明确行列，类似一维数组

// image-20220407210252194

// 部分赋初值

// image-20220407210331297
// image-20220407210413337
// image-20220407210419569
// image-20220407210427773
// 二维数组与指针
// C语言规定数组名代表数组首元素地址
...
// image-20220407210703663

// image-20220407210710233

// 总结

// image-20220407210736550
...
// 字符串指针变量只是一个指向字符串首地址的指针变量，我们可以对指针变量进行赋值，确定其指向的地址空间==（指针指向的字符串为字符串常量，不能再通过指针来修改其内容）==； 而字符串数组在定义时便在内存中为其分配了空间，也就是说，我们不能随意的改变这个数组的地址。
...
// 把src所指向的字符串（包括“\0”）复制到dest所指向的==字符串后面==（删除*dest原来末尾的“\0”）。要保证*dest足够长，以容纳被复制进来的*src。*src中原有的字符不变。返回指向dest的指针。
...
// 就是说它进行初始化时并不关心你要初始化的数组是什么类型的，它均以字节为单位进行初始化。 比如你的数组是int型的。int为4字节。

// 举例1：

// memset(a, 1, sizeof(a));
// ==在a中每个元素占 4字节== 比如int型的0为 0x00000000 ==int 型的最大值为 0x7FFFFFFF== 0xF = 0x1111 占四位 ==1字节为8bit ，所以两位为1字节== 使用memset进行初始化后会变为 0x01010101 也就是16843009 所以以下两种初始化效果是一样的
