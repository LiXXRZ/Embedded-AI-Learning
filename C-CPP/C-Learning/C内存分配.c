// static用法
// staic 修饰全局变量
// staic修饰全局变量后，该变量只能在本文件中使用而不允许在其他文件中使用。
// 这个全局变量被称为静态全局变量（存放在数据段，静态局部变量也放在这里），你可以把它当作本文件的私有变量。

// static 修饰局部变量
// static 修饰局部变量时，该变量称为静态局部变量
// 静态局部变量的存储空间在静态存储区（数据段，全局变量也放在这里），与函数的堆栈区不在同一个区域，因此函数运行完毕后静态局部变量不会被释放。

// 静态局部变量只赋一次初值。
// 下次使用静态局部变量的时候直接使用上次保存下来的值而不会重新赋值。

// staic 修饰函数
// 该函数只能被本文件的其他函数调用而不允许被其他文件的函数调用,这样的函数又叫作静态函数。
...
// 人类习惯的读写方式    12345678    0x12345678
// 大端模式    高地址     78 56 34 12      低地址
// 小端模式    高地址     12 34 56 78      低地址

// 在数据存储模式中，除了字节序，还有位序的概念。
// 位序指在一字节的存储中，各个比特位的存储顺序。
// 以十六进制数据0x78=01111000(B)为例，其在内存中可能有2种存储方式。
...
// 例，当我们定义一个结构体变量时，编译器会按照下面的原则在内存中给这个变量分配合适的存储空间。
// 结构体内各成员按照各自数据类型的对齐模数对齐。
// 结构体整体对齐方式：按照最大成员的size或其size的整数倍对齐。

// 结构体之所以要对齐，根本原因就是为了加快CPU访问内存的速度
// 在具体实现上，一般都采用每种数据类型的默认对齐模数sizeof(type)对齐。
// 不同的编译器有时候可能会采取不同的对齐标准
// 以GCC为例，GCC默认的最大对齐模数为4，当一种数据类型的大小超过4字节时会仍然按照4字节对齐
// 这是GCC和VC++6.0、Visual Studio、arm-linux-gcc等编译器不一样的地方。
...
// 在联合体中，我们定义一个长度为11的数组
// 在GCC编译环境下，联合体成员的最大对齐模数是4，所以整个联合体的大小是4的公倍数：4×3=12字节。

// 在arm-linux-gcc编译环境下，联合体成员的最大对齐模数是8，所以整个联合体的大小是8的公倍数：8×2=16字节。

// 内存对齐知识补充
// 结构体内当前元素要满足其下一个元素的对齐模数，按前方字节总数除下一元素对齐模数所得余数与其的差值在当前元素后添加字节。
...
// struct BBB {
//   long num;        //4  8    7980
//   char *name;      //8  8    7988   地址在64位系统中占8字节
...
// (1) 结构体第一个成员的偏移量（offset）为0，以后每个成员相对于结构体首地址的偏移量都是该成员大小与有效对齐值中较小那个的整数倍，如有需要编译器会在成员之间加上填充字节。
...
// 则结构体变量test_s所占的字节数变为12字节，即比原来的16字节省下了4个字节
...
// 4、堆(heap)
// 堆是用于存放进程运行中被动态分配的内存段,它的大小并不固定,可动态扩张或缩减。
...
// 注意： （1）这里不区分初始化和未初始化的数据区，是因为静态存储区内的变量若不显示初始化，则编译器会自动以默认的方式进行初始化，
...
// ###程序运行时分配（32位操作系统，每个程序独立4GB虚拟内存空间）
//  **系统空间（System Space）**
//  **命令行参数区**
//  - 用途：存储命令行传递的参数（如 `ls -l /home`）。
//  - 关联代码：`int main(int argc, char **argv)`。
//  **栈区（Stack）**
//  - 特性：存储局部变量，自动分配（`{}` 内有效），离开 `{}` 自动释放。
//  - 空间增长：由上往下增长。
//  - 初始值：未初始化的值为随机值。
//  **堆区（Heap）**
//  - 分配方式：通过 `malloc` 分配（需自行管理，用完需用 `free` 释放，否则会内存泄漏）。
//  - 空间增长：由下往上增长。
//  - 初始值：内存内容为随机值，一般用 `memset()` 清0。

// ### 编译出来的ELF文件
//  **数据段（Data Segment）**
//  - **.bss段**：
//    - 存储内容：未初始化的全局变量、初始化为0的全局变量或 `static` 变量。
//    - 初始值：默认为0。
//  - **.data段**：
//    - 存储内容：已经初始化的全局变量或 `static` 变量。
//  - **.rodata段**：
//    - 存储内容：`const`、`#define`、`char *ptr="string"` 等定义的数据常量。
//  **文本段（Text Segment, .txt）**：
//  - 用途：存储编译出来的可执行文件的二进制代码。
...
// 解析： 因为“hello”是常量，存放在.rodata段，不可改变。
...
// 定义：
// 命令行传递的参数:Is -l /home int main(int argc, char **argv)
...
//         char arr[] = "hello";
// }
// 未定义的局部变量，其值是随机值。 未初始化的静态局部变量，其值为0。
...
// 已初始化的全局变量，可以被所有其他c文件或本C文件中的函数调用，如果另一个c文件想调用全局变量，要用extern声明。 例：
...
// struct _st_student student1;                                 bss段
// int g_def_age = 10;                                          data段

// int main(int argc, char *argv)                               命令行参数区
// {
//     st_student student2;                                     栈区
//     st_student *student3;                                    栈区
//     static int count;                                        bss段
//     char *name = "zhangsan";                                 栈区/rodata段

//     student3 = (st_student *)malloc(sizeof(st_student));     栈区/堆区
//     if (NULL == student3)
//     {
//         printf("malloc() failure\n");
//         return -1;
//     }
//     memset(student3, 0, sizeof(*student3));
//     free(student3);
//     student3 = NULL;
//     return 0;
// }
