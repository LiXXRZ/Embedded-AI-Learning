// 预处理命令
// 在接触#if、#undef这类预处理指令前，大部分都都接触过#define、#include等预处理命令，
// 通俗来讲预处理命令的作用就是在编译和链接之前，对源文件进行一些文本方面的操作，比如文本替换、文件包含、删除部分代码等，这个过程叫做预处理（在编译之前对源文件进行简单加工）
...
// 还有需要注意的一点是，不管是在某个函数内，还是在所有函数之外（不太建议把#define写在函数内），
...
// 这里说下题外话
// #define叫宏定义，但是在笔者的认识里对声明和定义的理解是：
// 声明不分配内存，定义才分配内存，
// 所以#define虽然名字里面有“定义”两个字，但并不占存储空间（为什么不叫宏声明···）

// 总结：#define和声明、定义都不同，宏定义不占内存空间，因为宏在预处理阶段就会被替换掉，到了编译的阶段是没有宏存在的，它在预编译阶段就被处理了

// #undef
// 上文提到#define的作用域是从它声明开始到文件结尾，
// #undef就是取消之前的宏定义（也就是#define的标识符）
...
// #if命令要求判断条件为整型常量表达式，也就是说表达式中不能包含变量，而且结果必须是整数；
// 而if后面的表达式没有限制，只要符合语法就行,这是#if和if的一个重要区别
...
// 它的意思是，如果该宏已被定义过，则对程序段1进行编译，否则对程序段2进行编译
// （这个和上面的#if一样最后都需要#endif），上述格式也可以不用#else，这一点上和if else相同
...
// #endif
// #endif上面已经用过多次了，需要知道的就是**#endif是#if, #ifdef, #ifndef**这些条件命令的结束标志.

// 上面说了8种预处理命令，下面再补充一个预处理函数（注意是函数且该函数有返回值）

// defined函数
// defined函数的作用是判断某个宏是否被定义,若该宏被定义则返回1，否则返回0，
// 该函数经常与#if #elif #else配合使用，一般使用格式为：
