...
理想电感、电容只储能、不消耗平均功率；真实元件会因为线圈电阻、介质损耗、漏电等原因消耗能量。课件用品质因数 $Q$ 来衡量“储能能力相对损耗能力”的大小：

$$
Q=2\pi\frac{\text{峰值储能}}{\text{每周期损耗能量}}
$$

$Q$ 越大，说明损耗越小；通常 $Q>10$ 时，可以使用高 $Q$ 近似。
...
其中 $r_L$ 是线圈铜损等效出来的串联电阻。对应的品质因数为：

$$
Q_L=\frac{\omega L}{r_L}=\frac{X_L}{r_L}
$$

并联模型：

$$
Y_L=\frac1{R_L}+\frac1{j\omega L'}
$$

对应的品质因数为：

$$
Q_L'=\frac{R_L}{\omega L'}=\frac{R_L}{X_{L'}}
$$

串联模型和并联模型可以在某一指定频率下互相等效。精确换算为：

$$
R_L=\frac{r_L^2+(\omega L)^2}{r_L}=(1+Q_L^2)r_L
$$

$$
L'=\frac{r_L^2+(\omega L)^2}{\omega^2L}=L\left(1+\frac1{Q_L^2}\right)
$$

高 $Q$ 时：

$$
L'\approx L,\qquad R_L\approx Q_L^2r_L\approx Q_L\omega L
$$

也就是说，一个很小的串联损耗电阻，换成并联模型时会变成一个很大的并联电阻。
...
$$
Z_C=r_C+\frac1{j\omega C}=r_C-j\frac1{\omega C}
$$

其中 $r_C$ 常称为 ESR，即等效串联电阻。对应品质因数为：

$$
Q_C=\frac{|X_C|}{r_C}=\frac1{\omega C r_C}
$$

并联模型：

$$
Y_C=\frac1{R_C}+j\omega C'
$$

对应品质因数为：

$$
Q_C'=\omega C'R_C=\frac{R_C}{|X_{C'}|}
$$
...
$$
C'=\frac{C}{1+\frac1{Q_C^2}}
$$

高 $Q$ 时：

$$
C'\approx C,\qquad R_C\approx Q_C^2r_C\approx Q_C|X_C|
$$
...
$$
X=|X_L|\quad\text{或}\quad X=|X_C|
$$

高 $Q$ 时，串联损耗电阻、并联损耗电阻和品质因数之间有三条常用近似：
...
注意：这些公式都和当前频率有关，因为 $X_L=\omega L$，而 $|X_C|=1/(\omega C)$。不要把某个频率下算出的 $r$、$R$ 直接搬到另一个频率下使用。

### 4. 谐振题中怎样合并实际损耗

在串联谐振中，更习惯把损耗都折算成串联电阻。若实际电感和实际电容都有损耗，则在谐振频率附近：
...
$$
r_L=\frac{\rho}{Q_L},\qquad r_C=\frac{\rho}{Q_C}
$$
...
也就是损耗更大的那个元件，也就是 $Q$ 更低的那个元件，会主导整个谐振电路的 $Q$。

在并联谐振中，更习惯把损耗都折算成并联电阻。多个并联损耗支路会以电导形式相加：
...
- 串联谐振题：优先把实际电感、电容损耗折算成串联电阻 $r$；
- 并联谐振题：优先把损耗折算成并联电阻 $R$；
- 题目给 $L,Q$ 或 $C,Q$ 时，先用 $r\approx X/Q$ 找串联损耗；
- 题目要并联等效时，再用 $R\approx QX$ 或 $R\approx Q^2r$ 换过去；
- 高 $Q$ 近似本质是 $Q$ 很大时 $L'\approx L$、$C'\approx C$，不是无条件成立。

---
## 十二、负载与信号源对 $Q$ 的影响

源内阻和负载电阻都会消耗平均功率，因此都会降低谐振电路的品质因数 $Q$。课件给出的处理思路是：

> 对串联谐振，把所有损耗都折算成等效串联电阻；对并联谐振，把所有损耗都折算成等效并联电阻。
...
$$
\rho=\sqrt{\frac LC}=\omega_0L=\frac1{\omega_0C}
$$

而总损耗电阻为：

$$
r_{eq}=r_S+r_L+r_{\text{load}}+\cdots
$$

所以一旦源内阻或负载折算成了串联损耗，$r_{eq}$ 变大，$Q$ 就会下降；同时带宽
...
$$
Q_L=200,\qquad Q_C\gg Q_L
$$

$$
V_s=10\ \mathrm{mV}\ \text{(rms)},\qquad L=1\ \mathrm{mH},\qquad C=160\ \mathrm{pF}
$$
...
$$
\omega_0=\frac1{\sqrt{LC}}
=\frac1{\sqrt{10^{-3}\cdot160\times10^{-12}}}
...
$$
\rho=\sqrt{\frac LC}
=\sqrt{\frac{10^{-3}}{160\times10^{-12}}}
=2.5\times10^3\ \Omega
$$

#### 情况 1：只看 LC 自身

因为题目给 $Q_C\gg Q_L$，说明电容损耗远小于电感损耗，所以 LC 自身的品质因数主要由电感决定：
...
$$
r_{eq}=r_S+r_L=12.5+12.5=25\ \Omega
$$
...
$$
V_{C01}=Q_1V_s=100\times10\ \mathrm{mV}=1\ \mathrm{V}
$$

源内阻已经把 $Q$ 从 $200$ 拉低到 $100$。

#### 情况 3：接入负载 $R=250\ \mathrm{k}\Omega$

负载 $R$ 和电容支路并联。为了用串联谐振公式，需要把这个并联损耗折算成等效串联损耗。

在谐振频率处：

$$
|X_C|=\rho=2.5\ \mathrm{k}\Omega
$$
...
$$
Q_R=\frac{R}{|X_C|}
=\frac{250\ \mathrm{k}\Omega}{2.5\ \mathrm{k}\Omega}
=100
$$

因为 $Q_R$ 仍然较高，可以用高 $Q$ 换算：

$$
r_{\text{load}}\approx\frac{X_C^2}{R}
=\frac{(2.5\times10^3)^2}{250\times10^3}
=25\ \Omega
$$
...
$$
r_{eq}=r_S+r_L+r_{\text{load}}
=12.5+12.5+25=50\ \Omega
$$
...
$$
V_{C02}=Q_2V_s=50\times10\ \mathrm{mV}=0.5\ \mathrm{V}
$$

接上负载以后，电路仍然谐振，但选频能力变弱，电压放大也变小。

### 本题的核心变化

| 情况 | 总串联损耗 | $Q$ | 电容电压 |
| :--- | :--- | :--- | :--- |
| 只看 LC 自身 | $r_L=12.5\ \Omega$ | $200$ | 未计源时不直接用 |
| 加源内阻、空载 | $r_S+r_L=25\ \Omega$ | $100$ | $1\ \mathrm{V}$ |
| 加源内阻、带载 | $r_S+r_L+r_{\text{load}}=50\ \Omega$ | $50$ | $0.5\ \mathrm{V}$ |

一句话：**负载不是改变谐振频率的主角，它主要是增加损耗；损耗越大，$Q$ 越低，带宽越宽，谐振处的电压放大越弱。**
...
$$
B_\omega=\frac{\omega_0}{Q}=\frac1{RC}
$$

如果用普通频率，则：

$$
B_f=\frac{B_\omega}{2\pi}=\frac1{2\pi RC}
$$
...
所以并联谐振不是电压放大，而是**支路电流放大**：

$$
I_C=I_L=QI_s
$$

这里的 $I_C$ 和 $I_L$ 相位相反，大小相等，所以它们在外部看来基本互相抵消，电源只需要提供电阻损耗电流。

### 4. 输入阻抗的带通特性

若用电流源激励并联 RLC，则输出电压为：

$$
H(j\omega)=\frac{\dot V}{\dot I_s}=Z(j\omega)=\frac1{Y(j\omega)}
$$

代入导纳：

$$
H(j\omega)=\frac1{\frac1R+j\omega C+\frac1{j\omega L}}
$$
...
H(j\omega)=
\frac{j\omega/C}
{(j\omega)^2+\frac{j\omega}{RC}+\frac1{LC}}
$$

这说明并联谐振电路从“电流输入、电压输出”的角度看，是一个带通特性：
...
-j\frac{\omega L}{r^2+(\omega L)^2}
$$

整个并联电路导纳变为：

$$
Y=\frac{r}{r^2+(\omega L)^2}
+j\left(\omega C-\frac{\omega L}{r^2+(\omega L)^2}\right)
$$

谐振时令虚部为零：

$$
\omega C-\frac{\omega L}{r^2+(\omega L)^2}=0
$$
...
当 $Q_L$ 很高时，$1/Q_L^2$ 很小，所以：

$$
\omega_p\approx\omega_0
$$

这就是为什么高 $Q$ 条件下，仍然常用理想公式 $\omega_0=1/\sqrt{LC}$。

### 6. 实际电感损耗的并联等效

在高 $Q$ 下，实际电感的串联电阻 $r$ 可以换成并联损耗电阻：

$$
R\approx\frac{X_L^2}{r}
$$

在谐振附近 $X_L\approx\rho$，所以：

$$
R\approx\frac{\rho^2}{r}
=\frac{L}{Cr}
$$
...
$$
Q=\frac{R}{\rho}=\frac{\rho}{r}
$$

这个换算很常用：并联谐振题通常希望把所有损耗都折算成并联电阻，然后用
...
| 电源电流 | 最大 | 最小 |
| 功率因数 | $pf=1$ | $pf=1$ |
| 特性阻抗 | $\rho=\sqrt{L/C}$ | $\rho=\sqrt{L/C}$ |
| 品质因数 | $Q=\rho/r$ | $Q=R/\rho$ |
| 带宽 | $B_\omega=r/L$ | $B_\omega=1/(RC)$ |
| 放大现象 | $V_L=V_C=QV_s$ | $I_L=I_C=QI_s$ |
| 高频近似 | $Q>10,\ B\ll\omega_0$ | $Q>10,\ B\ll\omega_0$ |

高 $Q$ 时，两个截止角频率可以近似写成：

$$
\omega_{c1}\approx\omega_0-\frac{B}{2},\qquad
\omega_{c2}\approx\omega_0+\frac{B}{2}
$$

这个近似的意思是：带宽很窄时，两个半功率点几乎对称分布在 $\omega_0$ 两侧。
...
这里要注意带宽单位：本题先把谐振频率写成普通频率 $f_0$，所以后面带宽也用 Hz：
...
所以不是只能用 $f$ 或只能用 $\omega$，而是要前后一致：用 $f_0$ 算出来的是 Hz，用 $\omega_0$ 算出来的是 $\mathrm{rad/s}$。
