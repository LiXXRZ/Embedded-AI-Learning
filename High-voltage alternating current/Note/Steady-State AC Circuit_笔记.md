...
$$
P_L=\frac{V_S^2R_L}{(R_S+R_L)^2+(X_S+X_L)^2}
$$
...
| 负载相角固定，只能调模值 | $|Z_L|=|Z_S|$ | 受限条件下最大功率 |

所以课件例题中：

- 可任意调负载时，$Z_L=100+j100\ \Omega$，$P_{max}=100\ \mathrm{W}$；
- 负载只能是纯电阻时，$R_L=|Z_0|=100\sqrt2\ \Omega$，$P'_{max}\approx82.3\ \mathrm{W}$。
