...
这里的 `Re(\cdot)` 表示取实部，`Im(\cdot)` 表示取虚部。  
复功率把平均功率和无功功率打包在一起，所以：

$$
\operatorname{Re}(\dot S)=P,\qquad \operatorname{Im}(\dot S)=Q
$$

因此，算平均功率时就取复功率的实部；虚部对应无功交换，不是净消耗。
...
这道互感例题本质上是同一个耦合网络的两种观察角度。题目给的是 $X_M=1\ \Omega$，所以 $\omega M=1$。两种写法都对，只是“从哪一侧看进去”不同。

先把最容易混的两个原边电流分开记：

| 记号 | 场景 | 公式 | 用途 |
| :--- | :--- | :--- | :--- |
| $\dot I_{1,oc}$ | 副边开路，求副边戴维宁开路电压 | $\dot I_{1,oc}=\dot V_s/Z_{11}$ | 只用来求 $\dot V_{oc}$ |
| $\dot I_1$ | 负载接上后的真实原边电流 | $\dot I_1=\dot V_s/(Z_{11}+Z_{1r})$ | 用来算源电流、源功率 |
| $\dot I_1$ | 已经求出 $\dot I_2$ 后回算原边 | $\dot I_1=(\dot V_s+j\omega M\dot I_2)/Z_{11}$ | 和上一行是同一个最终电流 |

所以 $\dot V_s/Z_{11}$ 不是最终带负载的原边电流，它只是**副边开路那一步的辅助电流**。只要副边负载接上，副边电流就会通过互感反过来影响原边，此时必须用反映阻抗法或完整方程法求最终 $\dot I_1$。
...
$$
Z_{11}=j3-j1=j2,\qquad Z_{22}=1+j1.5-j0.5=1+j
$$
...
$$
\dot I_1=\frac{\dot V_s}{Z_{11}+Z_{1r}}
=\frac{10}{j2+\frac12-j\frac12}
=2-j6\ \mathrm{A}
$$
...
这一路最适合先问“源要供多少功率”或“原边电流是多少”。因为副边的影响已经被压成一个反映阻抗，原边方程最短。

#### 写法 B：先对副边做戴维宁等效

先把副边开路，得到开路电压。开路时副边电流为零，原边辅助电流是

$$
\dot I_{1,oc}=\frac{\dot V_s}{Z_{11}}
$$

所以副边开路电压为

$$
\dot V_{oc}=j\omega M\dot I_{1,oc}
=\frac{j\omega M}{Z_{11}}\dot V_s
=\frac{j\cdot10}{j3-j}
=5\ \mathrm{V}
$$
...
$$
Z_{2r}=\frac{\omega^2M^2}{Z_{11}}
=\frac{1}{j3-j}
=-j0.5\ \Omega
$$

于是副边看到的是一个戴维宁源，串联阻抗为
...
$$
\dot I_2=\frac{\dot V_{oc}}{Z_0+1}
=\frac{5}{-j0.5+j1.5-j0.5+1}
=4-j2\ \mathrm{A}
$$
...
$$
\dot I_1=\frac{\dot V_s+j\omega M\dot I_2}{Z_{11}}
=\frac{10+j(4-j2)}{j3-j}
=2-j6\ \mathrm{A}
$$

这一路最适合先问“副边负载电流是多少”或“副边吸收多少功率”，因为副边已经被整理成标准戴维宁形式。

#### 为什么两种写法都能成立

1. 这个耦合网络是线性的，端口电压电流关系是唯一的。  
2. 反映阻抗法和戴维宁法，只是把同一个二端口从不同端口压缩成更简单的等效形式。  
3. 只要点号和电流方向统一，算出的 $\dot I_1,\dot I_2$ 以及平均功率就必须一致。

#### 怎么选

- 要先算原边电流、源功率，优先用写法 A。  
- 要先算副边电流、负载功率，优先用写法 B。  
- 题目给了互感 $M$ 或 $X_M$ 时，两种写法都能走通，不是两道题，而是同一道题的两种切法。
## 十二、考前速记
