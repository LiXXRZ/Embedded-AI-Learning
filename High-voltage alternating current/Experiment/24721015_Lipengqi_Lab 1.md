...
<div align="center" style="width: 100%; height: 280mm; display: flex; align-items: center; justify-content: center; overflow: hidden; page-break-inside: avoid;">
  <img src="https://free.boltp.com/2026/04/10/69d873690ad0b.webp" style="max-width: 100%; max-height: 100%; object-fit: contain;" alt="Image 2">
</div>
...
### VII. Reflection and Conclusion
### 1.Ghosting of Lissajous Figures
#### a) Experimental Scenario and Description of Anomalies
* **Experimental Context:** This occurred during the final step of **Lab Section 3 (Verification of Mixed Dual-Frequency Signals and Filtering Characteristics)**. The objective was to magnify the output terminal ($v_8$) and measure the peak-to-peak voltage of the residual high-frequency ripples after passing through the low-pass filter (theoretical expected value is approximately 250mV).
* **Observed Anomalies:**
    1. **Data Anomaly:** The vertical voltage difference ($\Delta Y$) measured by the oscilloscope cursors (T1, T2) was as high as 2.658V, far exceeding the expected 0.25V.
    2. **Waveform Distortion I:** The mixed waveform at the input terminal ($v_5$) did not exhibit standard centrally symmetric oscillation, but rather appeared to be "climbing" along a steep slope.
    3. **Waveform Distortion II:** The waveform at the output terminal ($v_8$) appeared as a thick, flat line with no apparent fluctuations; the high-frequency sawtooth ripples were almost imperceptible to the naked eye.

#### b) Root Cause Analysis
After meticulous review and troubleshooting, it was confirmed that the circuit itself was operating perfectly. The aforementioned phenomena were all **typical measurement artifacts caused by improper instrument parameter settings and visual illusions**:

* **Cause 1: Cursor Misplacement (Addressing the Data Anomaly)**
  The binding target of the cursors was incorrect. During measurement, cursors T1 and T2 were mistakenly locked onto Channel A (Node $v_5$, i.e., the output of the intermediate adder). This node had not yet undergone filtering, so the high-frequency noise (31.8kHz) was fully preserved with a peak-to-peak value of 2.5V. The measured 2.658V was actually the superposition of the "unfiltered high-frequency peak-to-peak voltage" and the "voltage elevation of the low-frequency main wave over an extremely short time interval." This data could not be used to evaluate the filter's output performance.
* **Cause 2: "Baseline Tilt" Illusion Caused by Extreme Timebase Truncation (Addressing Waveform Distortion I)**
To clearly observe the high-frequency ripples, the oscilloscope's horizontal timebase was reduced to an extreme scale of **5.00 μs/Div**. However, the full period of the 1.59kHz low-frequency main wave is as long as **628 μs**. Within such a microscopic observation window, the oscilloscope merely captured a tiny "slice" of the rising edge of the low-frequency macroscopic sine wave. In a calculus sense, this tiny segment of the curve approximates a straight line, thereby creating the visual illusion of the high-frequency waveform "oscillating on a steep slope."

* **Cause 3: "Flat-Line" Illusion Caused by Limited Vertical Resolution (Addressing Waveform Distortion II)**
When observing Channel B at the output terminal, the Vertical Scale was still kept at the **1.00 V/Div** setting used for measuring the macroscopic waveform. For the weak ripple that only has about 0.25V remaining after filtering, it only occupies one-fourth of a major division on a 1V scale. Limited by the vertical quantization precision of the screen pixels, this minute fluctuation was visually compressed into a thick line with blurred edges.

#### c) Improvement Plan and Measurement Specifications
In response to these errors, we established the following strict specifications to be adhered to in future mixed-signal measurements:

* **Accurately Bind the Measurement Source:** Before using the Cursors or automatic Measure functions, it is imperative to double-check that the Source is bound to the correct target channel (e.g., CH2 or the $v_8$ terminal) to avoid the elementary mistake of "observing Channel A but measuring Channel B."
* **Apply a "Dynamic Scaling" Strategy to Isolate Signals:** When handling multi-frequency mixed signals, one must not attempt to view all information using a single scale setting.
    * **Macroscopic Observation (Low-Frequency Components):** The timebase must be expanded (e.g., to **200 μs/Div**) alongside a larger voltage scale to evaluate the overall attenuation and phase shift of the envelope.
    * **Microscopic Observation (High-Frequency Components):** The timebase must be reduced (e.g., to **50 μs/Div**), and **the vertical scale of that channel must be significantly lowered simultaneously** (e.g., down to **100 mV/Div** or **50 mV/Div**). Utilizing the oscilloscope's internal amplifier to vertically "stretch" the waveform is the only way to accurately extract the residual high-frequency features.




#### 2. Breadboard Parasitics and Grounding Practices
* **Experimental Scenario:** During the measurement of the high-frequency pole (approx. 30kHz) and the isolation of the dual-frequency mixed signal, we observed that the actual attenuation rate (6.77%) was consistently slightly higher than the theoretical extreme attenuation state.
* **Reflection and Analysis:** This occurs because a solderless breadboard is not an ideal connection medium. At high frequencies in the tens of kilohertz, non-negligible **Parasitic Capacitance** and **Parasitic Inductance** are formed between the parallel metal spring clips inside the breadboard, as well as between the cluttered flying wires inserted on the board. These parasitic parameters provide unexpected high-frequency "bypass" channels, preventing the high-frequency noise from being completely blocked by the RC filter. Furthermore, if the grounding points of the signal generator, operational amplifier power supply, and oscilloscope are not strictly unified (resulting in a **Ground Loop**), it is highly prone to introducing additional power-line interference.
* **Improvement Plan:** In future high-frequency circuit experiments, component leads should be trimmed as short as possible to keep the components flush with the breadboard surface. Flying wires should be kept brief and parallel crossings must be avoided. Most importantly, the strict **"Star Grounding"** principle must be adhered to in order to ensure the absolute consistency of all reference potentials.

#### 3. Trace Thickness Error and Noise Susceptibility in Lissajous Figures
* **Experimental Scenario:** When measuring the phase angle using the Lissajous-Figure Method in Lab Section 1, we found that measurement errors increased dramatically as the phase angle approached its limit (e.g., -90° at the high-frequency pole).
* **Reflection and Analysis:** A theoretical Lissajous figure is a geometric shape composed of infinitely thin lines. In empirical measurements, however, because the oscilloscope probes pick up the environmental **Noise Floor**, combined with the inherent **Trace Thickness** of the oscilloscope's display, the ellipse on the screen manifests as a "thick line" with a certain width and often blurred edges. When the phase angle approaches 90°, the ellipse becomes extremely narrow, and the physical thickness of the trace directly obscures the true Y-axis intercept ($X_0$). At this point, whether the cursors are placed on the inner edge, the center, or the outer edge of the thick line will result in a massive deviation when calculating the inverse sine value $\arcsin(X_0/X_{max})$.
* **Improvement Plan:** For measured signals containing background noise, before applying the Lissajous method, the oscilloscope's **"Average Acquisition Mode"** should be enabled (e.g., averaging 16 or 64 acquisitions). This significantly suppresses random noise, rendering the traces thinner and sharper, thereby greatly improving the geometric precision of the intercept readings.
