...
*   **频谱系数公式**：
    $$C_n = \frac{1}{T_0} \int_{0}^{T_0} \tilde{x}(t) e^{-jn\omega_0 t} dt$$
      ...
      *   **注意**：指数形式中会出现 $n<0$ 的负频率分量。对实信号而言，正负频率成分通常成对出现，并满足共轭对称；它们共同组成实际的实信号波形。
...
利用对称性可以极大地简化傅里叶级数的计算。下面几条结论建议和第十节一起记：

| 对称性类型 | 时域条件 | 频谱包含的成分 |
| :--- | :--- | :--- |
| **偶对称 (Even Symmetry)** | $\tilde{x}(t) = \tilde{x}(-t)$ | **仅含直流和余弦分量**，即 $b_n = 0$ |
| **奇对称 (Odd Symmetry)** | $\tilde{x}(t) = -\tilde{x}(-t)$ | **仅含正弦分量**，即 $a_n = 0,\ a_0 = 0$ |
| **半波重复 / 半波周期性** | $\tilde{x}(t) = \tilde{x}(t \pm T_0/2)$ | **若仍以 $T_0$ 为参考周期展开，则只含偶次谐波**；奇次谐波为 0。直流分量是否存在取决于平均值 |
| **半波反对称 / 半波镜像** | $\tilde{x}(t) = -\tilde{x}(t \pm T_0/2)$ | **只含奇次谐波**；偶次谐波和直流分量均为 0 |

补充说明：

- “半波重复”表示前后半周期完全相同，本质上说明信号的实际周期可以缩短为 $T_0/2$。若还按 $T_0$ 展开，就会表现为“只剩偶次谐波”。
- “半波反对称”是教材中更常见的名称，也常被写成“半波镜像”或“奇谐对称”；它并不等同于奇函数，但结论是**无直流、无偶次、只留奇次**。
- 某些波形在平移或去除直流分量后，会进一步呈现奇对称、偶对称或半波对称特性。
- 详细推导与对比总结见第十节。
...
## 十、半波重复与半波反对称信号的傅里叶级数特性详解

### (1) 半波重复信号 (Half-Wave Repetition Signal)
#### 1. 定义
半波重复信号满足：
...
#### 3. 傅里叶级数特性
**结论：若仍以 \(T_0\) 作为参考周期展开，则半波重复信号只包含偶次谐波（\(n=2,4,6,\cdots\)），奇次谐波分量为 0；但直流分量是否存在，取决于信号平均值，并不必然为 0。**
...
- 直流分量 $n=0$：$F_0 = \frac{1}{T_0}\int_{T_0} \widetilde{x}(t)dt = \frac{2}{T_0}\int_{0}^{T_0/2} \widetilde{x}(t)dt$，只有当半周期平均值为 0 时才消失。

换句话说，半波重复信号真正表达的是：它的**实际周期已经是 \(T_0/2\)**。  
若还强行用 \(T_0\) 当周期来写傅里叶级数，就会看到“只剩偶次谐波”。

### (2) 半波反对称信号 (Half-Wave Anti-Symmetry Signal)
#### 1. 定义
半波反对称信号（也叫**半波镜像**、**奇谐对称**）满足：
$$\widetilde{x}(t) = -\widetilde{x}\left(t \pm \frac{T_0}{2}\right)$$
...
#### 3. 傅里叶级数特性
**结论：半波反对称周期信号的傅里叶级数，只包含奇次谐波（$n=1,3,5\cdots$），没有直流分量（$n=0$），也没有偶次谐波分量。**
...
1. **半波反对称信号 ≠ 奇函数**
   ...
   - 半波反对称信号是“平移半个周期后反号”，二者是不同的对称性，可同时满足（如方波）。

2. **工程意义**
   - 半波反对称信号：常见于交流电力系统中的**奇次谐波**（如3次、5次谐波），是电网谐波分析的核心；
   - 半波对称信号：常见于倍频电路、分频电路，用于提取偶次谐波分量。

### (5) 快速记忆口诀
- 半波重复：**半周重合，只留偶次**
- 半波镜像：**半周反号，只留奇次**
- 直流判断：**半波重复看平均值，半波镜像必无直流**
