...
## 零、 核心思想：离散时间的傅里叶级数
> **💡 物理直觉与核心区别**：
> 1.  对**离散周期信号**而言，时域只需一个周期内的 \(N\) 个样本就能完全描述，因此频域也只需要 \(N\) 个彼此独立的基函数。
...
$$
e^{j\frac{2\pi}{N}mk},\qquad m=0,1,\cdots,N-1
$$

若把 \(m\) 增加 \(N\)，则：

$$
e^{j\frac{2\pi}{N}(m+N)k} =
e^{j\frac{2\pi}{N}mk}e^{j2\pi k} =
e^{j\frac{2\pi}{N}mk}
$$

因为 \(k\) 是整数，\(e^{j2\pi k}=1\)。  
所以 \(m\) 与 \(m+N\) 对应的是同一个基函数，这就是 DFS 频谱周期为 \(N\) 的根本原因。
...
对周期序列而言，判断“偶”“奇”时要按模 \(N\) 的方式理解：

- 偶对称：\(\tilde{x}[k]=\tilde{x}[-k]=\tilde{x}[N-k]\)
- 奇对称：\(\tilde{x}[k]=-\tilde{x}[-k]=-\tilde{x}[N-k]\)
...
### 📝 例题 2：直接按定义计算 DFS (Slide 9)

**【题目】** 周期 \(N=4\)，一个周期内

$$
\tilde{x}[k]=\{\underline{1},2,3,4\}
$$
...
$$
\tilde{X}[m] =
\sum_{k=0}^{3}\tilde{x}[k]e^{-j\frac{2\pi}{4}mk}
$$
...
$$
\tilde{X}[1] =
1+2e^{-j\frac{\pi}{2}}+3e^{-j\pi}+4e^{-j\frac{3\pi}{2}}
=-2+2j
$$

$$
\tilde{X}[2] =
1+2e^{-j\pi}+3e^{-j2\pi}+4e^{-j3\pi}
=-2
$$

$$
\tilde{X}[3] =
1+2e^{-j\frac{3\pi}{2}}+3e^{-j3\pi}+4e^{-j\frac{9\pi}{2}}
=-2-2j
$$
...
### 📝 例题 3：巧用欧拉公式“逆向匹配法”求余弦序列 (Slide 10) 🌟 极易考！
...
### 📝 例题 5：巧用线性性质求组合序列 (Slide 13)
**【题目】** 周期 \(N=3\)，

$$
\tilde{x}[k]=\{\underline{2},1,1\}
$$
...
$$
\tilde{x}[k] =
\{\underline{0},1,1\} + 2\{\underline{1},0,0\}
$$
...
### 📝 例题 7：超高能考点——用“矩阵法”秒杀周期卷积 (Slide 27)
