...
>
>**（3）另解：**
>解答该题需要结合周期信号的傅里叶级数展开以及线性时不变系统（LTI）的频域滤波特性。以下是严谨的推导过程：
>>
>**1. 滤波器特性分析**
>由题目已知，该理想模拟低通滤波器的频率响应为：
>$$ H(j\omega) = \begin{cases} e^{-j2\omega}, & |\omega| < 2\pi \\ 0, & \text{其他} \end{cases} $$
>- **通带区域**：$(-2\pi, 2\pi)$，即截止频率 $\omega_c = 2\pi$。
>- **幅度特性**：在通带内 $|H(j\omega)| = 1$。
>- **相位特性**：在通带内 $\phi(\omega) = -2\omega$。根据时发定理 $\mathscr{F}\{f(t-t_0)\} = F(j\omega)e^{-j\omega t_0}$，该相位特性表现为对通过的信号产生 $t_0 = 2$ 的纯时间延迟。
>
>**2. 输入信号 $x_3(t)$ 的频域分析**
>输入信号 $x_3(t)$ 为周期矩形波，从0到1为1，从1到2为-1。
>- **周期**：$T = 2$。
>- **基波角频率**：$\omega_0 = \frac{2\pi}{T} = \pi$。
>- **直流分量**：一个周期内的平均值为 0，即 $a_0 = 0$。
>
>将 $x_3(t)$ 展开为三角形式的傅里叶级数：
>$$ a_n = \frac{2}{T}\int_{0}^{T} x_3(t)\cos(n\omega_0 t) dt = \int_{0}^{1}\cos(n\pi t)dt - \int_{1}^{2}\cos(n\pi t)dt = 0 $$
>$$ b_n = \frac{2}{T}\int_{0}^{T} x_3(t)\sin(n\omega_0 t) dt = \int_{0}^{1}\sin(n\pi t)dt - \int_{1}^{2}\sin(n\pi t)dt = \frac{2[1-\cos(n\pi)]}{n\pi} $$
>
>当 $n$ 为偶数时，$b_n = 0$；
>当 $n$ 为奇数时，$b_n = \frac{4}{n\pi}$。
>
>因此，$x_3(t)$ 的傅里叶级数展开式为：
>$$ x_3(t) = \frac{4}{\pi}\sin(\pi t) + \frac{4}{3\pi}\sin(3\pi t) + \frac{4}{5\pi}\sin(5\pi t) + \cdots $$
>信号包含的频率成分为 $\omega = \pi, 3\pi, 5\pi, \cdots$。
>
>**3. 滤波过程与输出计算**
>将 $x_3(t)$ 通过截止频率为 $\omega_c = 2\pi$ 的低通滤波器：
>- **基波分量** $\frac{4}{\pi}\sin(\pi t)$：其频率为 $\pi$，满足 $\pi < 2\pi$，位于滤波器通带内，允许通过。
>- **三次及以上谐波分量**（$3\pi, 5\pi, \cdots$）：由于均大于 $2\pi$，完全落在滤波器阻带内，被彻底衰减截断（乘以 0）。
>
>因此，只有基频分量能够通过系统。对该分量附加滤波器的相移（即时间延迟 $t=2$）：
>$$ y_3(t) = \frac{4}{\pi}\sin[\pi(t - 2)] $$
>
>化简该三角函数：
>$$ y_3(t) = \frac{4}{\pi}\sin(\pi t - 2\pi) = \frac{4}{\pi}\sin(\pi t) $$
>系统输出为：
>$$ y_3(t) = \frac{4}{\pi}\sin(\pi t) $$
